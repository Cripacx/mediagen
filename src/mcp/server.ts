...
/**
 * Marking is its own tool because it is its own step: it runs on a file that
 * already exists, after someone has looked at it. Generation does not mark.
 */
const MARK_SHAPE = {
  filePath: z.string().describe('The media file to mark'),
  visibleLabel: z
    .boolean()
    .optional()
    .describe(
      "Composite the European Commission's official AI label into a copy. The original is never overwritten",
    ),
  modified: z
    .boolean()
    .optional()
    .describe('Claim the media was altered by a model rather than generated by one'),
  labelPosition: z
    .enum(LABEL_POSITIONS)
    .optional()
    .describe(
      'Which corner the visible label sits in. Defaults to bottom-right; "auto" picks whichever corner has the least detail',
    ),
  provider: z.string().optional().describe('Record which provider produced it'),
  model: z.string().optional().describe('Record which model produced it'),
}
...
export function buildServer(): McpServer {
  ...
  server.registerTool(
    'mark_media',
    {
      title: 'Mark media as AI-generated',
      description:
      ...
      try {
        const result = await markFile(
          args.filePath,
          {
            ...
            visibleLabel: args.visibleLabel === true,
            labelKind: args.modified === true ? 'modified' : 'generated',
            ...(args.labelPosition === undefined ? {} : { labelPosition: args.labelPosition }),
            ...(args.visibleLabel === true ? { outputPath: labelledPathFor(args.filePath) } : {}),
          },
          {
            ...(args.provider === undefined ? {} : { provider: args.provider }),
            ...(args.model === undefined ? {} : { model: args.model }),
